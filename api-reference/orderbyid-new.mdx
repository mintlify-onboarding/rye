...
> 📘
> 
> When called for a not submitted cart, [Order.status](/reference/orderstatus) will always be `PENDING` and [Order.events](/reference/orderevent) array will be empty. After cart submission [Order.status](/reference/orderstatus) will change according to the current checkout state and the [Order.events](/reference/orderevent) array will be populated.
...
The ID of the [Order](/reference/order-new)

---

### Returns

[Order.\*](/reference/order-new): [Order](/reference/order-new)

Any requested field from the [Order](/reference/order-new) object.
